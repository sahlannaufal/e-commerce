...
function UserProvider({ children }: Props) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/users/authenticate/${eCommerceToken}`);
  ...
  const logIn = async ({ email, password }: LogInProps) => {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/users/`, {
      email,
      password,
    });
  ...
  const signUp = async (user: UserProps) => {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/create/user`, user);
