...
function DeleteCategory({ categories }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/delete/categories/${categoryName}`);
...
export async function getServerSideProps() {
  const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/categories`);
